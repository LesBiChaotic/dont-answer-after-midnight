  ...
  return (
    <div className="flex-1 flex flex-col app-viewport bg-night-bg text-night-text">
      ...
      <main className="flex-1 overflow-y-auto p-4 space-y-4 max-w-md mx-auto w-full font-mono text-xs">
        ...
        <div className="p-4 bg-night-surface border border-night-border rounded-3xl space-y-3 font-sans">
          <h3 className="text-xs font-semibold text-white font-mono">SIMULATION_TRIGGERS</h3>

          <div className="flex gap-2">
            <button
              type="button"
              onClick={() => navigate('/notebook')}
              className="flex-1 py-2.5 px-3 bg-brand-600/80 hover:bg-brand-600 text-white rounded-xl text-xs font-semibold flex items-center justify-center gap-1.5 min-h-touch"
            >
            ...
            <button
              ...
              className="flex-1 py-2.5 px-3 bg-night-card hover:bg-night-hover border border-night-border text-brand-300 rounded-xl text-xs font-semibold flex items-center justify-center gap-1.5 min-h-touch"
            >
          ...
          <button
            ...
            className="w-full py-3 px-4 bg-night-card hover:bg-night-hover border border-night-border text-white rounded-xl text-xs font-semibold flex items-center justify-center gap-2 min-h-touch active:scale-95 transition-all shadow-xs"
          >
        ...
        <div className="p-4 bg-night-surface border border-night-border rounded-3xl space-y-3 font-sans">
          <div className="flex items-center justify-between text-xs">
            <span className="font-semibold text-white font-mono">STORY_STAGE_OVERRIDE</span>
        ...
        <div className="p-4 bg-night-surface border border-night-border rounded-3xl space-y-3">
          ...
          <div className="space-y-2">
            {continuityRecords.map((rec) => (
              <div
                key={rec.id}
                className="p-3 bg-black/40 border border-night-border rounded-xl text-[11px] space-y-1 text-emerald-400"
              >
                <div className="flex items-center justify-between text-[10px] text-night-muted">
        ...
        <div className="p-4 bg-night-surface border border-night-border rounded-3xl space-y-2">
          <span className="font-semibold text-white">ACTIVE_THREAD_PARTITIONS</span>
          <div className="space-y-1 text-[11px] text-night-muted">
            {threads.map((t) => (
              <div key={t.id} className="flex items-center justify-between p-2 bg-night-card rounded-lg">
                <span className="text-white truncate">{t.title || t.id}</span>
