      ...
      <main className="flex-1 overflow-y-auto p-4 space-y-4">
        ...
        {readableMode ? (
          ...
          <div className="space-y-4 max-w-xl mx-auto select-text font-mono">
            ...
            {currentEra === '2001' && (
              <div className="bg-ah-surface-2 border-2 border-[#243352] rounded-xl p-3 text-ah-text space-y-3 shadow-lg">
                <div className="bg-ah-surface-2 p-2.5 border border-[#314368] text-center rounded-lg">
                  <div className="text-xs font-bold text-[#69C49A] tracking-widest uppercase">
                    === MIDNIGHT BOARD v2.1 [CGI/PERL] ===
                  </div>
                  <div className="text-[10px] text-ah-muted">DIAL-UP RELAY NODE #1 // 2400-33600 BPS OK</div>
                </div>

                {messages.map((msg, i) => (
                  <table key={msg.id} className="w-full text-xs border border-[#243352] bg-ah-surface-2 mb-2 rounded overflow-hidden">
                    <thead>
                      <tr className="bg-ah-surface-2 text-ah-muted text-[10px]">
                    ...
                    <tbody>
                      <tr>
                        <td className="p-2 border-r border-[#243352] align-top text-[11px] bg-ah-surface-2">
                          <div className="font-bold text-[#69C49A]">{msg.senderDisplayName}</div>
                          <div className="text-[10px] text-ah-muted">@{msg.senderHandle}</div>
                          <div className="text-[9px] text-[#F0A06D] mt-2">SYS_MEMBER #0{i + 12}</div>
                        </td>
                        <td className="p-2.5 align-top text-[12px] leading-relaxed text-ah-text">
                          {msg.content}
                        </td>
                      </tr>
                    </tbody>
            ...
            {currentEra === '2004' && (
              <div className="bg-ah-surface-2 border border-[#1C3342] rounded-2xl p-3.5 text-[#57C7C1] font-mono text-xs space-y-2 shadow-lg">
            ...
            {currentEra === '2008' && (
              <div className="bg-ah-surface-2 border border-[#3E2563] rounded-2xl p-3.5 text-[#B979FF] font-mono text-xs space-y-3 shadow-lg">
                <div className="flex items-center justify-between border-b border-[#3E2563] pb-2 text-[10px] text-[#8197FF]">
                  ...
                  <span className="bg-ah-surface-2 px-2 py-0.5 rounded-full text-[#B979FF]">PROTOCOL: HUSH-P2P</span>
                </div>

                {messages.map((msg) => (
                  <div key={msg.id} className="p-3 bg-ah-surface-2 border border-[#482A75] rounded-xl space-y-1 shadow-sm">
            ...
            {currentEra === '2013' && (
              <div className="bg-ah-surface-2 border border-[#452D5A] rounded-3xl p-3.5 space-y-3 font-sans shadow-lg">
                <div className="bg-ah-surface-2 p-3 rounded-2xl flex items-center justify-between text-xs text-ah-text shadow-xs">
                ...
                {messages.map((msg) => (
                  <div key={msg.id} className="p-3 bg-ah-surface-2 border border-[#3D2557] rounded-2xl space-y-1.5 shadow-sm">
            ...
            {currentEra === '2018' && (
              <div className="bg-ah-surface-2 border border-[#2B1E3D] rounded-3xl p-4 space-y-3 font-sans shadow-lg">
                ...
                {messages.map((msg) => (
                  <div key={msg.id} className="p-3.5 bg-ah-surface-2 border border-[#392454] rounded-2xl space-y-1 shadow-sm">
                    <div className="flex items-center justify-between text-xs">
